...
HEADLESS = not os.environ.get('DISPLAY') or os.environ.get('SSH_CONNECTION')

# Conditional imports - handle missing picamera2 gracefully
try:
    ...
    print("Install it with: sudo apt-get install python3-picamera2")
...
class Camera:
    def __init__(self, camera_num=0, preview_size=(640, 480), hflip=True, vflip=True, stream_size=(400, 300)):
        if not PICAMERA2_AVAILABLE:
            raise ImportError("picamera2 is not available. Install it with: sudo apt-get install python3-picamera2")
        
        try:
            self.camera = Picamera2(camera_num)              # Initialize the Picamera2 object
            ...
            raise RuntimeError(f"Failed to initialize camera {camera_num}: {e}. Make sure camera is connected and enabled.")

    def start_image(self):
        # Only start preview if we have a display (not headless)
        if not HEADLESS and Preview is not None:
            try:
                self.camera.start_preview(Preview.QTGL)  # Start the camera preview using the QTGL backend
            except Exception as e:
                print(f"Warning: Could not start preview (running headless?): {e}")
        else:
            print("Running headless - preview disabled, camera will still capture images")
        self.camera.start()                      # Start the camera

    def save_image(self, filename):
        if not self.camera.started:
            self.camera.start()  # Start camera if not already started
