...
class Led:
    def __init__(self):
        ...
        # Import modules conditionally to avoid import errors when libraries aren't installed
        if self.pcb_version == 1 and self.pi_version == 1:
            # PCB v1 + Pi < 5: Use WS281X library
            try:
                from rpi_ledpixel import Freenove_RPI_WS281X
            except ImportError as e:
                print(f"Error importing rpi_ledpixel: {e}")
                print("Install rpi_ws281x library: sudo pip3 install rpi-ws281x")
            ...
            try:
                ...
                print(f"Error initializing WS281X LED strip: {e}")
                print("Make sure SPI is enabled and rpi_ws281x library is installed.")
                self.is_support_led_function = False

        elif self.pcb_version == 2 and (self.pi_version == 1 or self.pi_version == 2):
            # PCB v2: Use SPI-based LED driver
            try:
                from spi_ledpixel import Freenove_SPI_LedPixel
            except ImportError as e:
                error_msg = str(e)
                print(f"Error importing spi_ledpixel: {error_msg}")
                if "numpy" in error_msg.lower():
                    print("Missing required package: numpy")
                    print("Install it with: sudo pip3 install numpy")
                elif "spidev" in error_msg.lower():
                    print("Missing required package: spidev")
                    print("Install it with: sudo pip3 install spidev")
                else:
                    print("Make sure spi_ledpixel.py is in the same directory.")
                    print("Also ensure required packages are installed:")
                    print("  sudo pip3 install numpy spidev")
                self.is_support_led_function = False
                return
            
            try:
                self.strip = Freenove_SPI_LedPixel(4, 255, 'GRB')
                # Check if SPI initialization was successful
                if hasattr(self.strip, 'check_spi_state') and self.strip.check_spi_state() == 0:
                    print("Warning: SPI LED initialization failed. SPI may not be enabled.")
                    print("Enable SPI by running: sudo raspi-config -> Interface Options -> SPI -> Enable")
                    print("Or add 'dtparam=spi=on' to /boot/firmware/config.txt and reboot.")
                    self.is_support_led_function = False
                else:
                    self.is_support_led_function = True
            except Exception as e:
                print(f"Error initializing SPI LED strip: {e}")
                print("Make sure SPI is enabled:")
                print("  sudo raspi-config -> Interface Options -> SPI -> Enable")
                print("  Or add 'dtparam=spi=on' to /boot/firmware/config.txt and reboot")
                print("  Also ensure spidev and numpy are installed: pip3 install spidev numpy")
                self.is_support_led_function = False
